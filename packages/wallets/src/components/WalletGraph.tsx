...
import { Color, mojoToChik, mojoToCAT, blockHeightToTimestamp } from '@chik-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(
        0,
        ([WalletType.CAT, WalletType.CRCAT].includes(walletType) ? mojoToCAT(start) : mojoToChik(start)).toNumber(),
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip: [WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(start)
        : mojoToChik(start).toString(), // bignumber is not supported by react
    });
  });
