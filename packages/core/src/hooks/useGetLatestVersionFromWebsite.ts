...
export default function useGetLatestVersionFromWebsite(): UseGetLatestVersionFromWebsiteResult {
  ...
  const [latestVersionURL] = useLocalStorage<string>(
    'latestVersionURL',
    'https://download.chik-network.net/latest/latest.json'
  );
    ...
    ipcRenderer.invoke('fetchHtmlContent', latestVersionURL).then((obj: any) => {
      try {
        ...
        /* we don't need to handle error here, if we are unable to fetch version number
           from chik-network.net, we just ignore showing reminder dialog */
      }
    });
  }, [latestVersionURL]);

  const downloadUrl = downloadPath ? new URL(downloadPath, 'https://www.chik-network.net/').toString() : undefined;
  const releaseNotesUrl = releaseNotesPath ? new URL(releaseNotesPath, 'https://www.chik-network.net/').toString() : undefined;
  const blogUrl = blogPath ? new URL(blogPath, 'https://www.chik-network.net/').toString() : undefined;
