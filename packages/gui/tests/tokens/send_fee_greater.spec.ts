...
test('Confirm that User cannot send a TXCK fee greater then in Wallet', async () => {
  ...
  await page.locator('text=Fee *TXCK >> input[type="text"]').fill('10');
  ...
  await expect(page.locator('text=Pending Change0 TXCK')).toBeVisible();
});
