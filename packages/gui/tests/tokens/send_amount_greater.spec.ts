...
test('Confirm that User cannot send a TXCK amount greater then in Wallet', async () => {
  let receive_wallet = 'txck1ksr59en3j3t9zaprmya0jp9k6mkazq2u5lfccaf85sej7kgmvjjscanfxd';
  ...
  console.log(`XCK Balance: ${balance}`);
  ...
  //await page.locator('text=Fee *TXCK >> input[type="text"]').fill('0.000005');
  ...
  await expect(page.getByText('Pending Balance0 TXCK')).toBeVisible();
});
