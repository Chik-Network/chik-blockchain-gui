...
const getChikVersion = () => {
  let version = null;
  const exePath = getExecutablePath('chik');
  // first see if we can get a chik exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or chik is on the path
    try {
      version = childProcess
