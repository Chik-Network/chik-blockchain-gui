...
export type PermissionsPairRecord = {
  ...
  fingerprint: number;
  /** Wire form `chik_<name>`. Granted at pairing; empty = deny-all. */
  commands: string[];
 ...
 * after a paired dapp's `chik_showNotification` passed the gate. Renderer
...
type PermissionsService = {
  ...
  registerPair: (payload: {
    ...
    mainnet: boolean;
    metadata: PermissionsPairMetadata;
    /** WC commands from the dapp's session proposal, wire form `chik_<name>`. */
    commands: string[];
