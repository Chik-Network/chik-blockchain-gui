...
function isChikCacheInfoFile(filePath: string) {
  return isChikCacheFile(filePath) && (filePath.endsWith(INFO_SUFFIX) || filePath.endsWith(INFO_TEMP_SUFFIX));
}

// A file a write is streaming into, data or sidecar; stale once no write is.
function isChikCacheTempFile(filePath: string) {
  return (
...
export default class CacheManager extends EventEmitter {
  ...
  private async removeStaleTempFiles() {
    ...
    await Promise.all(
      files
        .filter((file) => isChikCacheTempFile(file))
  ...
  private async performEviction(targetSize: number, preserveFilePath?: string): Promise<void> {
    ...
    files.filter(isChikCacheFile).forEach((file) => {
      const suffix = isChikCacheInfoFile(file)
