    ...
    <div className="flex flex-col h-screen bg-chik-bg text-chik-text text-base">
      <div className="flex-1 min-h-0 overflow-y-auto px-7 pt-4 pb-4 space-y-3">
        <div className="flex items-start gap-4">
          <div className="shrink-0 w-12 h-12 rounded-xl bg-chik-primary-soft text-chik-primary flex items-center justify-center text-xl font-bold uppercase">
            {(metadata.name || '?').charAt(0)}
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="m-0 text-2xl font-semibold leading-tight text-chik-text truncate">
              {metadata.name || 'Unknown application'}
            </h1>
            {hasUrl && <div className="mt-0.5 text-sm text-chik-text-secondary truncate">{metadata.url}</div>}
            <p className="mt-0.5 mb-0 text-sm leading-snug text-chik-text-secondary">
              {isEdit
        ...
        <section className="relative z-30">
          <div className="rounded-xl border border-chik-border bg-chik-card">
            <div className="px-5 pt-2.5 pb-1.5 text-xs font-semibold uppercase tracking-wider text-chik-text-muted">
              {i18n._(/* i18n */ { id: 'Wallet keys' })}
            </div>
            {isEdit ? (
              <div className="flex items-center gap-3 mx-3 mb-3 px-3 py-2 rounded-md border border-chik-border-strong bg-chik-bg">
                {selectedFingerprint ? (
                  <>
                    <span className="shrink-0 w-[18px] h-[18px] rounded-full border-2 border-chik-primary flex items-center justify-center">
                      <span className="w-2 h-2 rounded-full bg-chik-primary" />
                    </span>
                    <div className="flex-1 min-w-0 text-sm truncate">
                      <span className="font-medium text-chik-text">{selectedWalletLabel}</span>
                      <span className="text-chik-text-muted font-mono ml-2">({selectedFingerprint})</span>
                    </div>
                  </>
                ) : (
                  <div className="text-sm text-chik-text-secondary">
                    {i18n._(/* i18n */ { id: 'No wallet key selected.' })}
                  </div>
                )}
              </div>
            ) : wallets.length === 0 ? (
              <div className="px-5 pb-3 text-sm text-chik-text-secondary">
                {i18n._(/* i18n */ { id: 'No wallet keys available.' })}
              </div>
            ) : (
              <details className="group relative">
                <summary className="flex items-center gap-2 mx-3 mb-3 px-3 py-2 rounded-md border border-chik-border-strong bg-chik-bg cursor-pointer list-none select-none hover:border-chik-primary transition-colors">
                  <div className="flex-1 flex flex-wrap items-center gap-1.5 min-w-0">
                    {wallets.map((wallet) => (
                      <span
                        ...
                        className="inline-flex items-center text-sm px-2 py-0.5 rounded bg-chik-primary-soft text-chik-text font-medium max-w-[200px] truncate"
                      >
                    ...
                    <span
                      ...
                      className="text-sm text-chik-text-secondary"
                    >
                      {i18n._(/* i18n */ { id: 'Select a wallet key…' })}
                    </span>
                  </div>
                  <svg
                    className="shrink-0 w-4 h-4 text-chik-text-secondary transition-transform group-open:rotate-180"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    aria-hidden="true"
                ...
                <div className="absolute left-3 right-3 top-full -mt-2 z-40 rounded-md border border-chik-border-strong bg-chik-surface shadow-xl max-h-60 overflow-y-auto">
                  <ul className="m-0 p-0 list-none divide-y divide-chik-border">
                    {wallets.map((wallet) => {
                      const checked = wallet.fingerprint === selectedFingerprint;
                      return (
                        <li key={wallet.fingerprint}>
                          <label className="flex items-center gap-3 px-4 py-2 cursor-pointer hover:bg-chik-card-elevated transition-colors">
                            <input
                              type="radio"
                              name="fingerprint"
                              ...
                              className="w-[18px] h-[18px] accent-chik-primary cursor-pointer shrink-0"
                            />
                            <span className="flex-1 min-w-0 text-sm truncate">
                              <span className="font-medium text-chik-text">
                                {wallet.label || `Wallet ${wallet.fingerprint}`}
                              </span>
                              <span className="text-chik-text-muted font-mono ml-2">({wallet.fingerprint})</span>
                            </span>
                          </label>
                        </li>
                      );
        ...
        {(capabilityGroups.length > 0 || allowed.length > 0) && (
          <section className="rounded-xl border border-chik-border bg-chik-card overflow-hidden">
            {capabilityGroups.length > 0 && (
              <>
                <div className="px-5 pt-2.5 pb-1.5 text-xs font-semibold uppercase tracking-wider text-chik-text-muted">
                  {i18n._(/* i18n */ { id: 'Allow without asking' })}
                </div>
                <ul className="m-0 p-0 list-none divide-y divide-chik-border border-t border-chik-border">
                  {capabilityGroups.map((group) => (
                    <li key={group.key}>
                      <label className="flex items-center gap-3 px-5 py-1.5 cursor-pointer hover:bg-chik-card-elevated transition-colors">
                        <input
                          type="checkbox"
                          defaultChecked={group.defaultChecked}
                          data-cap-toggle={group.key}
                          className="w-[18px] h-[18px] accent-chik-primary shrink-0"
                        />
                        <span className="flex-1 min-w-0 text-sm leading-snug">
                          <span className="font-medium text-chik-text">{group.label}</span>
                          <span className="text-chik-text-secondary">
                            {': '}
                            {group.description}
                          </span>
                        </span>
            ...
            {allowed.length > 0 && (
              <details className="group border-t border-chik-border">
                <summary className="flex items-center justify-between gap-2 px-5 py-2 cursor-pointer list-none select-none hover:bg-chik-card-elevated transition-colors">
                ...
                <div className="border-t border-chik-border">
                  {capabilityGroups.map((group) => (
                    <PerCommandGroup
                      key={group.key}
                      label={group.label}
                  ...
                  {hasConfirmationRequiredCommands && (
                    <div className="border-b border-chik-border last:border-b-0">
                      <div className="px-5 pt-2 pb-1 text-xs font-semibold uppercase tracking-wider text-chik-text-muted">
                  ...
                  {rejected.length > 0 && (
                    <div className="border-b border-chik-border last:border-b-0">
                      <div className="px-5 pt-2 pb-1 text-xs text-chik-text-muted">
                      ...
                      <div className="px-5 pb-2 flex flex-wrap gap-1.5">
                        {rejected.map((dappCommand) => (
                          <span
                            key={dappCommand}
                            title={dappCommand}
                            className="inline-flex items-center text-xs px-2 py-0.5 rounded border border-chik-border-strong text-chik-text-secondary font-medium"
                          >
      ...
      <div className="flex justify-end gap-2.5 px-7 py-4 border-t border-chik-border bg-chik-bg">
        <button
          type="button"
          data-action="cancel"
          className="px-5 py-2 text-sm font-semibold uppercase tracking-wider rounded-md border border-chik-primary bg-transparent text-chik-primary hover:bg-chik-primary-soft transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-chik-primary"
        >
        ...
        <button
          ...
          className="px-5 py-2 text-sm font-semibold uppercase tracking-wider rounded-md border border-transparent bg-chik-primary hover:bg-chik-primary-hover text-[#0f252a] shadow-sm transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-chik-primary focus-visible:ring-offset-2 focus-visible:ring-offset-chik-bg"
        >
...
function ReadOnlyCommandGroup({ label, commands }: { label: string; commands: string[] }) {
  ...
  return (
    <div>
      <div className="px-5 pt-2 pb-1 text-xs text-chik-text-muted">{label}</div>
      <div className="px-5 pb-2 flex flex-wrap gap-1.5">
  ...
  return (
    <div className="border-b border-chik-border last:border-b-0">
      ...
      <ul className="m-0 p-0 list-none">
        {commands.map((dappCommand) => (
          <li key={dappCommand}>
            <label className="flex items-center gap-3 px-5 py-1 cursor-pointer hover:bg-chik-card-elevated transition-colors">
              <input
                type="checkbox"
                ...
                className="w-[18px] h-[18px] accent-chik-primary cursor-pointer shrink-0"
              />
              <span className="text-sm text-chik-text font-mono truncate" title={dappCommand}>
                {humanizeDappCommandName(dappCommand)}
              </span>
            </label>
          </li>
