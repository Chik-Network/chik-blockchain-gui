...
import chikEnvironment, { chikInit } from './utils/chikEnvironment';
  ...
  async (payload: {
    ...
    try {
      ...
      const result = await dispatchPairRequest(
        ...
        // process the command
        async (context) => {
          const { destination, command: chikCommand } = parseCommandId(commandId);
          ...
          const response = dappCommandSchema.handler
            ...
            : await sendCommand(chikCommand, destination, parsedParams);
...
if (ensureSingleInstance() && ensureCorrectEnvironment()) {
  ...
  const createWindow = async () => {
    ...
    ipcMainHandle(AppAPI.SET_BYPASS_COMMANDS, async (commands: string[]) => {
      const allowedDestinations = ['chik_wallet', 'chik_full_node', 'chik_farmer', 'chik_harvester', 'daemon'];
    ...
    function setNetworkPrefix(newNetworkPrefix: string) {
      ...
      const isTestnet = networkPrefix === 'txck';
      const title = isTestnet ? 'Chik Blockchain (Testnet)' : 'Chik Blockchain';
    ...
    webSocketBridgeBindEvents(mainWindow.webContents, {
      ...
      onSend: async (_id: string, data: string) => {
        ...
        if (['chik_wallet.get_private_key'].includes(commandId)) {
          throw new Error('Private key is not allowed to be sent to the renderer process');
        }
  ...
  ipcMainHandle(ChikLogsAPI.SET_PATH, async () => {
  ...
  ipcMainHandle(ChikLogsAPI.GET_CONTENT, async () => {
    try {
      const currentPrefs = readPrefs();
  ...
  ipcMainHandle(ChikLogsAPI.GET_INFO, async () => {
    try {
    ...
    },
    {
      ...
      submenu: [
        ...
        },
        {
          ...
          click: () => {
            openExternal('https://x.com/chik_project');
          },
        },
      ],
    },
