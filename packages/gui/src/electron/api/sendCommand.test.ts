...
describe('sendCommand', () => {
  it('registers once and reuses the daemon socket for subsequent commands', async () => {
    ...
    await expect(sendCommand('get_network_info', 'chik_wallet')).resolves.toMatchObject({
      success: true,
      command: 'get_network_info',
    });
    await expect(sendCommand('get_logged_in_fingerprint', 'chik_wallet')).resolves.toMatchObject({
      success: true,
  ...
  it('routes concurrent responses by request id', async () => {
    ...
    const first = sendCommand('first_command', 'chik_wallet');
    const second = sendCommand('second_command', 'chik_wallet');
  ...
  it('parses unsafe integer response values as native bigints', async () => {
    ...
    const response = await sendCommand<{ mojos: unknown }>('get_wallet_balance', 'chik_wallet');
  ...
  it('stringifies bigint command values as JSON numbers', async () => {
    ...
    await sendCommand('send_bigint', 'chik_wallet', {
  ...
  it('ignores broadcast events with long-precision floats while a command is pending', async () => {
    await startServer((socket, message) => {
      ...
      // Raw frame mimicking a chik_harvester farming_info broadcast: the
      ...
      socket.send(
        '{"ack":false,"command":"farming_info","data":{"challenge_hash":"0xabc","total_plots":1000,"time":1.7656183690123726},"destination":"wallet_ui","origin":"chik_harvester","request_id":"broadcast"}',
      );
      socket.send(JSONBig.stringify({ request_id: message.request_id, data: { success: true, value: 'done' } }));
    });
    const sendCommand = loadSendCommand();

    await expect(sendCommand('get_offer_summary', 'chik_wallet')).resolves.toMatchObject({ value: 'done' });
  });
  ...
  it('resolves a matched response that contains long-precision floats', async () => {
    ...
    const response = await sendCommand<{ fee_rate: unknown }>('get_fee_estimate', 'chik_full_node');
  ...
  it('ignores frames that are not valid JSON while a command is pending', async () => {
    ...
    await expect(sendCommand('get_wallets', 'chik_wallet')).resolves.toMatchObject({ value: 'done' });
  });
  ...
  it('ignores broadcast events with long-precision floats during service registration', async () => {
    await startServer((socket, message) => {
      if (message.command === 'register_service') {
        socket.send(
          '{"ack":false,"command":"farming_info","data":{"time":1.2395747610135004},"destination":"wallet_ui","origin":"chik_harvester","request_id":"broadcast"}',
        );
      }
      socket.send(JSONBig.stringify({ request_id: message.request_id, data: { success: true, value: 'done' } }));
    });
    ...
    await expect(sendCommand('get_network_info', 'chik_wallet')).resolves.toMatchObject({ value: 'done' });
  });
  ...
  it('rejects daemon command errors', async () => {
    ...
    await expect(sendCommand('send_transaction', 'chik_wallet')).rejects.toThrow('daemon rejected command');
  });
  ...
  it('rejects an in-flight command when the socket closes and reconnects on the next command', async () => {
    ...
    await expect(sendCommand('close_me', 'chik_wallet')).rejects.toThrow('Connection closed before receiving response');
    await expect(sendCommand('after_close', 'chik_wallet')).resolves.toMatchObject({ reconnected: true });
