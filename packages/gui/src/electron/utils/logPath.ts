...
/**
 * Returns the default Chik debug log path derived from CHIK_ROOT (or the
 * standard `~/.chik/mainnet` location). This is the only log path used when the
...
export function getDefaultLogPath(): string {
  ...
  return path.join(chikRoot, 'log', 'debug.log');
}
 ...
 * be set through the native file picker in `ChikLogsAPI.SET_PATH`), the value on
