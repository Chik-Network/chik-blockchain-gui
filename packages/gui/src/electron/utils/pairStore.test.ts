...
describe('pairStore bypass reset', () => {
  it('adds a bypass command to a pair and refreshes updatedAt', () => {
    jest.spyOn(Date, 'now').mockReturnValue(2000);
    seedPairs([makePair({ bypass: ['chik_takeOffer'] })]);
    const store = loadStore();

    store.addBypassCommand('topic-1', 'chik_sendTransaction');

    expect(store.findPair('topic-1')).toMatchObject({
      bypass: ['chik_takeOffer', 'chik_sendTransaction'],
      updatedAt: 2000,
    });
  });

  it('does not duplicate an existing bypass command', () => {
    seedPairs([makePair({ bypass: ['chik_takeOffer'], updatedAt: 100 })]);
    const store = loadStore();

    store.addBypassCommand('topic-1', 'chik_takeOffer');
  ...
  it('throws when adding a bypass command for a missing pair', () => {
    ...
    expect(() => store.addBypassCommand('missing', 'chik_takeOffer')).toThrow('Pair not found: missing');
  });
  ...
  it('clears bypass permissions for all pairs', () => {
    ...
    seedPairs([
      makePair({ topic: 'topic-1', bypass: ['chik_takeOffer'] }),
      makePair({ topic: 'topic-2', bypass: ['chik_sendTransaction'] }),
    ]);
    const store = loadStore();
