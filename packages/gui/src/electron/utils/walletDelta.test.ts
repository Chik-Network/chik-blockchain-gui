...
describe('createOfferToWalletDelta', () => {
  ...
  it('rejects xck keys in create-offer params because create offers use numeric wallet IDs', () => {
    expect(() =>
      createOfferToWalletDelta({
        xck: '-4',
      }),
    ).toThrow('XCK is not a valid asset ID');
  });
...
describe('offerSummaryToWalletDelta', () => {
  it('converts a production-shaped XCK-for-CAT offer summary into taker wallet perspective', () => {
  ...
  it('allows summaries that include xck in both offered and requested maps as separate wallet directions', () => {
    expect(
  ...
  it('rejects numeric XCK asset keys in offer summaries', () => {
    expect(() =>
      offerSummaryToWalletDelta({
        offered: { '1': '4' },
        requested: {},
  ...
  it('converts requested XCK when offered does not also contain xck', () => {
