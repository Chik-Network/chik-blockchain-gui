...
describe('dispatchPairRequest authorization', () => {
  it('rejects requests for unknown pair topics before any user confirmation or processing', async () => {
    ...
    await expectWcError(
      dispatchPairRequest('missing-topic', 'chik_sendTransaction', {}, processRequest, confirmRequest),
      'Pair not found',
  ...
  it('rejects commands that were not granted to the pair', async () => {
    seedPairs([makePair({ commands: ['chik_getSyncStatus'] })]);
    const dispatchPairRequest = loadDispatchPairRequest();
    const { processRequest, confirmRequest } = makeHandlers('processed');

    await expectWcError(
      dispatchPairRequest('topic-1', 'chik_sendTransaction', {}, processRequest, confirmRequest),
  ...
  it('rejects missing, zero, malformed, and ungranted fingerprints', async () => {
    ...
    await expectWcError(
      dispatchPairRequest('topic-1', 'chik_sendTransaction', {}, jest.fn(), jest.fn()),
    ...
    await expectWcError(
      dispatchPairRequest('topic-1', 'chik_sendTransaction', { fingerprint: 0 }, jest.fn(), jest.fn()),
      'Fingerprint not allowed for this command',
      WcErrorCode.UNAUTHORIZED_METHOD,
    );

    await expectWcError(
      dispatchPairRequest('topic-1', 'chik_sendTransaction', { fingerprint: '123456' }, jest.fn(), jest.fn()),
      'Fingerprint not allowed for this command',
      WcErrorCode.UNAUTHORIZED_METHOD,
    );

    await expectWcError(
      dispatchPairRequest('topic-1', 'chik_sendTransaction', { fingerprint: 999_999 }, jest.fn(), jest.fn()),
      'Fingerprint not allowed for this command',
      WcErrorCode.UNAUTHORIZED_METHOD,
    );
  });
  ...
  it('allows an explicit granted fingerprint when it matches the logged-in fingerprint', async () => {
    ...
    await expect(
      dispatchPairRequest('topic-1', 'chik_sendTransaction', { fingerprint: 123_456 }, processRequest, confirmRequest),
  ...
  it('rejects a dapp command when it tries to change the active fingerprint', async () => {
    mockGetLoggedInFingerprint.mockResolvedValue(999_999);
    seedPairs([makePair({ commands: ['chik_getPublicKey'], fingerprint: 123_456 })]);
    const dispatchPairRequest = loadDispatchPairRequest();
    const { processRequest, confirmRequest } = makeHandlers('processed');

    await expectWcError(
      dispatchPairRequest('topic-1', 'chik_getPublicKey', { fingerprint: 123_456 }, processRequest, confirmRequest),
      'Fingerprint not allowed for this command',
      WcErrorCode.UNAUTHORIZED_METHOD,
    );
  ...
  it('rejects a dapp command after production param parsing preserves a different fingerprint as a number', async () => {
    ...
    const dispatchPairRequest = loadDispatchPairRequest();
    const { processRequest, confirmRequest } = makeHandlers('processed');
    const parsedParams = parseDappParams('chik_getPublicKey', JSONbig.stringify({ fingerprint: 123_456 }));

    await expectWcError(
      dispatchPairRequest('topic-1', 'chik_getPublicKey', parsedParams, processRequest, confirmRequest),
      'Fingerprint not allowed for this command',
      WcErrorCode.UNAUTHORIZED_METHOD,
    );
  ...
  it('executes bypassed commands only after pair, command, network, and fingerprint checks pass', async () => {
    seedPairs([makePair({ bypass: ['chik_sendTransaction'] })]);
    const dispatchPairRequest = loadDispatchPairRequest();
  ...
  it('does not let bypass skip command grants', async () => {
    seedPairs([makePair({ commands: ['chik_getSyncStatus'], bypass: ['chik_sendTransaction'] })]);
    const dispatchPairRequest = loadDispatchPairRequest();
    const processRequest = jest.fn(async () => 'processed');
  ...
  it('does not let bypass skip network checks', async () => {
    seedPairs([makePair({ mainnet: false, bypass: ['chik_sendTransaction'] })]);
    const dispatchPairRequest = loadDispatchPairRequest();
    const processRequest = jest.fn(async () => 'processed');
    const confirmRequest = jest.fn(async () => true);
  ...
  it('does not let bypass skip fingerprint checks', async () => {
    mockGetLoggedInFingerprint.mockResolvedValue(999_999);
    seedPairs([makePair({ bypass: ['chik_sendTransaction'], fingerprint: 123_456 })]);
    const dispatchPairRequest = loadDispatchPairRequest();
    const processRequest = jest.fn(async () => 'processed');
    const confirmRequest = jest.fn(async () => true);
  ...
  it('executes bypassed commands without manual confirmation', async () => {
    ...
    await expect(
      dispatchPairRequest('topic-1', 'chik_sendTransaction', { amount: '25', fee: 5 }, processRequest, confirmRequest),
    ).resolves.toBe('processed');
  ...
  it('rejects when the command is granted but the user denies manual confirmation', async () => {
    seedPairs([makePair({ commands: ['chik_sendTransaction'] })]);
    const dispatchPairRequest = loadDispatchPairRequest();
    const processRequest = jest.fn(async () => 'processed');
    const confirmRequest = jest.fn(async () => false);
