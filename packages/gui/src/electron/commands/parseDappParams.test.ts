...
describe('parseDappParams', () => {
  describe('parameter allowlist and defaults', () => {
    it('rejects unknown dapp commands before parsing params against a schema', () => {
      expect(() => parseDappParams('chik_unknownCommand', serialize({}))).toThrow(
        'Unknown dapp command: chik_unknownCommand',
      );
    });
  ...
  describe('fingerprint handling', () => {
    ...
    it('preserves and parses fingerprint for public-key commands that declare it', () => {
      expect(parseDappParams('chik_getPublicKey', serialize({ fingerprint: 123_456 }))).toEqual({
        fingerprint: 123_456,
      });
    });
  });
  ...
  describe('number params', () => {
    ...
    it('rejects number params that cannot be parsed as numbers', () => {
      expect(() => parseDappParams('chik_getPublicKey', serialize({ fingerprint: 'not-a-number' }))).toThrow(
  ...
  describe('bigint params', () => {
    ...
    it('accepts large bare JSON integer values as native bigint without precision loss', () => {
      expect(
        parseDappParams('chik_sendTransaction', '{"amount":9007199254740993,"fee":"0","address":"txck1address"}'),
      ).toMatchObject({
        amount: 9_007_199_254_740_993n,
        fee: 0n,
        address: 'txck1address',
  ...
  describe('malformed payloads', () => {
    it('rejects malformed JSON strings', () => {
      expect(() => parseDappParams('chik_getWallets', '{not-json')).toThrow();
    });

    it('rejects primitive JSON payloads before dispatch', () => {
      expect(() => parseDappParams('chik_getWallets', 'null')).toThrow();
      expect(() => parseDappParams('chik_getWallets', '1')).toThrow();
      expect(() => parseDappParams('chik_getWallets', '"string"')).toThrow();
    });
  });
});
