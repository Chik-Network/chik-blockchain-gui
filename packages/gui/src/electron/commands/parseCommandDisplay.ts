...
type AssetDisplayKind = 'chik' | 'wallet' | 'cat' | 'nft' | 'unknown';
...
function offerSummaryAssetKinds(offerSummary: OfferSummaryForDisplay): AssetDisplayKinds {
  ...
  for (const assetId of Object.keys(offerSummary.requested)) {
    const parsedAssetId = assetId === 'xck' ? '1' : assetId;
...
async function parseWalletDeltaItem(
  ...
  if (key === 'xck') {
    throw new Error('XCK is not a valid asset ID');
  }

  if (assetKind === 'chik' || key === '1') {
    return {
      kind: 'xck',
      amount: mojoToChikLocaleString(value),
    };
  }
...
function formatAmountWithRoyalties(
  ...
  if (line.kind === 'xck') {
    return mojoToChikLocaleString(totalAmount);
  }
...
async function walletDeltaToDisplay(
  ...
  return {
    ...
    fee: fee !== undefined ? mojoToChikLocaleString(fee) : undefined,
  };
}

export async function parseCommandDisplay(command: string, params: Record<string, unknown>) {
  if (command === 'chik_wallet.take_offer') {
  ...
  if (command === 'chik_wallet.create_offer_for_ids') {
