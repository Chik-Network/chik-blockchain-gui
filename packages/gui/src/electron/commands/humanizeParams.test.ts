...
describe('humanizeParams', () => {
  it('returns humanized visible params in schema order with labels', async () => {
    const params = [
      ...
      buildParam('fee', { label: () => 'Fee', humanize: 'mojo-to-xck', type: 'bigint' }),
      buildParam('memo', {
        isOptional: true,
        label: () => {
    ...
    await expect(humanizeParams(params, data, 'txck')).resolves.toEqual([
      {
        field: 'address',
        label: 'Address',
        value: 'txck1address',
  ...
  it('decodes a clawback puzzle decorator into a human-readable timelock', async () => {
    ...
    const rows = await humanizeParams(Commands['chik_wallet.send_transaction'].params, data, 'txck');
