...
function formatMojoXck(amount: unknown, networkPrefix?: string): string {
  const formatted = mojoToChikLocaleString(amount as string | number);
...
export async function humanizeParamValue(
  ...
  if (humanize) {
    switch (humanize) {
      case 'mojo-to-xck':
        return formatMojoXck(value, networkPrefix);
      case 'mojo-to-cat':
