...
describe('parseCommandDisplay', () => {
  ...
  it('displays raw create offer signs as spending and receiving wallet deltas', async () => {
    mockGetWalletInfos.mockResolvedValue({
      1: { name: 'Chik', type: WalletType.STANDARD_WALLET },
      6: { name: 'Test CAT', type: WalletType.CAT },
    });

    await expect(
      parseCommandDisplay('chik_wallet.create_offer_for_ids', {
        offer: {
          1: -10_000,
          6: 2_323_000,
        },
  ...
  it('fails closed when create-offer params use xck keys instead of numeric wallet IDs', async () => {
    mockGetWalletInfos.mockResolvedValue({});
    ...
    ).rejects.toThrow('XCK is not a valid asset ID');
  });

  it('normalizes take-offer xck summary keys to XCK display rows', async () => {
    mockGetWalletInfos.mockResolvedValue({});
    ...
    await expect(
      parseCommandDisplay('chik_wallet.take_offer', {
        offer: 'offer1...',
      }),
    ).resolves.toMatchObject({
      walletDelta: {
  ...
  it('omits the preview for a video NFT without a metadata preview image', async () => {
    ...
    const result = await parseCommandDisplay('chik_wallet.take_offer', {
      offer: 'offer1...',
    });
  ...
  it('fails closed if a raw xck asset key reaches display parsing', async () => {
    const entriesSpy = jest
      .spyOn(Object, 'entries')
      .mockReturnValueOnce([['1', '-1']])
      .mockReturnValueOnce([['xck', 1n]])
      .mockReturnValueOnce([]);
