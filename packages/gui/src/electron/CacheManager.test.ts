...
describe('CacheManager eviction', () => {
  ...
  beforeEach(async () => {
    ...
    cacheDirectory = await fs.mkdtemp(path.join(os.tmpdir(), 'chik-cache-manager-'));
  });
  ...
  it('keeps a completed download cached when cache housekeeping fails', async () => {
    ...
    jest
      ...
      .mockRejectedValueOnce(new Error("ENOENT: no such file or directory, stat '/cache/other-chikcache'"));
  ...
  it('ignores files that vanish while the cache size is being measured', async () => {
    ...
    await fs.writeFile(path.join(cacheDirectory, 'aaaa-chikcache'), Buffer.alloc(100));
    // a broken symlink stats like a file deleted between readdir and stat
    await fs.symlink(path.join(cacheDirectory, 'missing-target'), path.join(cacheDirectory, 'bbbb-chikcache'));
  ...
  it('evicts without failing when a file vanishes during the eviction scan', async () => {
    ...
    await fs.writeFile(path.join(cacheDirectory, 'aaaa-chikcache'), Buffer.alloc(200));
    await fs.symlink(path.join(cacheDirectory, 'missing-target'), path.join(cacheDirectory, 'bbbb-chikcache'));

    await expect(cacheManager.setMaxCacheSize(100)).resolves.toBeUndefined();
    await expect(fs.stat(path.join(cacheDirectory, 'aaaa-chikcache'))).rejects.toThrow('ENOENT');
  });
  ...
  it('starts the retry count over when a transient failure follows a settled one', async () => {
    ...
    // failures only
    await fs.writeFile(
      path.join(cacheDirectory, `${urlHash}-chikcache-info`),
  ...
  it('sweeps leftover temp files at startup', async () => {
    const stale = path.join(cacheDirectory, 'aaaa-chikcache.tmp');
    await fs.writeFile(stale, Buffer.alloc(300));
    await fs.writeFile(path.join(cacheDirectory, 'bbbb-chikcache'), Buffer.alloc(100));
    ...
    await expect(fs.stat(path.join(cacheDirectory, 'bbbb-chikcache'))).resolves.toBeDefined();
  ...
  it('counts temp files toward the cache size and removes them with the cache', async () => {
    ...
    const temp = path.join(cacheDirectory, 'aaaa-chikcache.tmp');
  ...
  it('evicts a stale temp file but never the one a download in flight is writing', async () => {
    ...
    const inFlightTemp = (await fs.readdir(cacheDirectory)).find(
      (file) => file.endsWith('.tmp') && file !== 'aaaa-chikcache.tmp',
    );
  ...
  it('reports a sidecar that cannot be read by its error code, not its path', async () => {
    ...
    await fs.mkdir(path.join(cacheDirectory, `${urlHash}-chikcache-info`));
...
describe('CacheManager request options from the renderer', () => {
  ...
  beforeEach(async () => {
    mockDownloadFile.mockReset();
    cacheDirectory = await fs.mkdtemp(path.join(os.tmpdir(), 'chik-cache-options-'));
  });
...
describe('CacheManager sidecar integrity and error redaction', () => {
  ...
  beforeEach(async () => {
    mockDownloadFile.mockReset();
    cacheDirectory = await fs.mkdtemp(path.join(os.tmpdir(), 'chik-cache-sidecar-'));
  });
  ...
  it('keeps the cache directory out of a persisted download error and out of what the renderer reads', async () => {
    mockDownloadFile.mockRejectedValue(
      Object.assign(new Error(`EACCES: permission denied, open '${cacheDirectory}/abc-chikcache.tmp'`), {
        code: 'EACCES',
      }),
    );
    const cacheManager = new CacheManager({ cacheDirectory, maxCacheSize: 1024 });
...
describe('CacheManager directory scans', () => {
  ...
  beforeEach(async () => {
    mockDownloadFile.mockReset();
    cacheDirectory = await fs.mkdtemp(path.join(os.tmpdir(), 'chik-cache-scan-'));
  });
  ...
  it('stats a large cache a bounded number of files at a time', async () => {
    await Promise.all(
      Array.from({ length: 300 }, (_, i) => fs.writeFile(path.join(cacheDirectory, `f${i}-chikcache`), 'x')),
    );
    const cacheManager = new CacheManager({ cacheDirectory, maxCacheSize: 1024 * 1024 });
    await cacheManager.init();
...
describe('CacheManager cache: responses', () => {
  ...
  beforeEach(async () => {
    mockDownloadFile.mockReset();
    cacheDirectory = await fs.mkdtemp(path.join(os.tmpdir(), 'chik-cache-protocol-'));
  });
  ...
  it('does not run per-download housekeeping for downloads it aborted', async () => {
    ...
    await Promise.all(
      Array.from({ length: FILES }, async (_, index) => {
        const base = path.join(cacheDirectory, `${index.toString(16).padStart(8, '0')}-chikcache`);
