...
function FarmingRewardsHistoryCards() {
  ...
  const totalChikFarmedCard = useMemo(() => {
    if (!data || isLoading) {
      return <CardSimple title={<Trans>Total XCK Farmed</Trans>} value="-" loading={isLoading} error={error} />;
    }
    ...
    if (data && data.farmedAmount) {
        ...
        <StyledTable>
          <tbody>
            <tr>
              ...
              <td>
                {mojoToChikLocaleString(data.poolRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToChikLocaleString(data.farmerRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToChikLocaleString(data.feeAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
      ...
      cardValue = (
        <Tooltip title={title}>
          {mojoToChikLocaleString(data.farmedAmount, locale)}
          &nbsp;
          {currencyCode}
        </Tooltip>
      );
    ...
    return <CardSimple title={<Trans>Total XCK Farmed</Trans>} value={cardValue} loading={isLoading} error={error} />;
