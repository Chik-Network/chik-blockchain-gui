import { useGetBlockchainStateQuery, useGetTotalHarvestersSummaryQuery } from '@chik-network/api-react';
import { State, CardSimple, useCurrencyCode, mojoToChikLocaleString, useLocale } from '@chik-network/core';
...
const MOJO_PER_CHIK = 1_000_000_000_000;
const BLOCKS_PER_YEAR = 1_681_920; // 32 * 6 * 24 * 365
function getBlockRewardByHeight(height: number) {
  if (height === 0) {
    return 21_000_000 * MOJO_PER_CHIK;
  }
  if (height < 3 * BLOCKS_PER_YEAR) {
    return 2 * MOJO_PER_CHIK;
  }
  if (height < 6 * BLOCKS_PER_YEAR) {
    return 1 * MOJO_PER_CHIK;
  }
  if (height < 9 * BLOCKS_PER_YEAR) {
    return 0.5 * MOJO_PER_CHIK;
  }
  if (height < 12 * BLOCKS_PER_YEAR) {
    return 0.25 * MOJO_PER_CHIK;
  }

  return 0.125 * MOJO_PER_CHIK;
}
...
function FarmingRewardsCards() {
  ...
  const estimatedDailyXCKCard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated daily XCK</Trans>} state={state} />;
    }

    const estimatedDailyXCK = new BigNumber(86_400)
    ...
    return (
      <CardSimple
        title={<Trans>Estimated daily XCK</Trans>}
        value={
          <>
            {mojoToChikLocaleString(estimatedDailyXCK, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  const estimatedMonthlyXCKCard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated monthly XCK</Trans>} state={state} />;
    }

    const estimatedMonthlyXCK = new BigNumber(86_400 * 31)
      .div(expectedTimeToWinSeconds)
      .multipliedBy(getBlockRewardByHeight(data.peak.height))
      .dp(0);
    ...
    return (
      <CardSimple
        title={<Trans>Estimated monthly XCK</Trans>}
        value={
          <>
            {mojoToChikLocaleString(estimatedMonthlyXCK, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  return (
    <Box>
      ...
      <Grid spacing={2} alignItems="stretch" container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          {estimatedDailyXCKCard}
        </Grid>
        <Grid xs={12} sm={6} md={4} item>
          {estimatedMonthlyXCKCard}
        </Grid>
      </Grid>
    </Box>
  );
