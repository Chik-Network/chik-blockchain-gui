import { resolveAppTheme, ThemeProvider, useColorModeValue, useDarkMode, useThemeVariant } from '@chik-network/core';
import { Overview as OverviewIcon } from '@chik-network/icons';
import {
...
function PagePreview({ page }: { page: string }) {
  const theme = useTheme();
  const rows = [
    'Primary content area keeps the original Chik function reachable.',
          ...
          <Box key={item} sx={{ py: 1.5, borderTop: `1px solid ${alpha(theme.palette.text.primary, 0.1)}` }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box flexGrow={1}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Preview-only row. Real Chik logic stays outside this browser sandbox.
                </Typography>
              </Box>
              <LinearProgress
                variant="determinate"
...
function SandboxScreen() {
  ...
  const metrics = [
    { label: 'Field Balance', value: '1,248.42 XCK', detail: 'Mock display data', icon: AccountBalanceWallet },
  ...
  const overviewSections = [
    { label: 'Wallets', value: '1,248.42 XCK', detail: 'Spendable balance, pending tx, address tools' },
        ...
        >
          <Toolbar sx={{ minHeight: 84, gap: 2, px: 3 }}>
            <Box flexGrow={1} minWidth={0}>
              <Typography variant="overline" color="text.secondary">
                Chik GUI design sandbox / Overview first
              </Typography>
