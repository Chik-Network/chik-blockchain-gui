...
export const WalletConnectChikProjectId = 'f3f661fcfc24e2e6e6c6f926f02c9c6e';
...
function processError(error: Error) {
  if (error.message.includes('No matching key')) {
    console.info('[chik-gui:walletConnect] Pairing not found (stale key, safe to ignore):', error.message);
    return;
  }
...
function parsePair(topic: string, mainnet: boolean, proposal: SignClientTypes.EventArguments['session_proposal']) {
  ...
  const requiredChik = requiredNamespaces?.chik;
  const optionalChik = optionalNamespaces?.chik;

  if (!requiredChik && !optionalChik) {
    throw new Error('Missing required chik namespace');
  }

  const chains = [...new Set([...(requiredChik?.chains ?? []), ...(optionalChik?.chains ?? [])])];

  const currentChain = mainnet ? 'chik:mainnet' : 'chik:testnet';
  ...
  const commands = [...new Set([...(requiredChik?.methods ?? []), ...(optionalChik?.methods ?? [])])];
...
export default function WalletConnectProvider(props: WalletConnectProviderProps) {
  ...
  const mainnet = useCurrencyCode() === 'XCK';
