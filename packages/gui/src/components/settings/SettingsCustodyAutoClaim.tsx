import { useGetAutoClaimQuery, useSetAutoClaimMutation } from '@chik-network/api-react';
import { Flex, SettingsText, Form, ButtonLoading, Fee, chikToMojo, mojoToChik } from '@chik-network/core';
import { ConnectCheckmark } from '@chik-network/icons';
...
export default function SettingsCustodyAutoClaim(props) {
  ...
  const autoClaimFee = autoClaimData?.txFee ? mojoToChik(autoClaimData.txFee).toNumber() : 0;
  ...
  async function handleSubmit({ fee }: { fee: number }) {
    const feeInMojos = chikToMojo(fee);
    await setAutoClaim({
      enabled: fee > 0,
      txFee: feeInMojos,
