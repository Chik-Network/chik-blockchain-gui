import type { DataLayerOfferSummary } from '@chik-network/api';
import { OfferSummaryValidTimes } from '@chik-network/api';
import {
  useGetWalletsQuery,
...
function OfferBuilderViewer(props: OfferBuilderViewerProps, ref: any) {
  ...
  const dataLayerOfferMethods = useForm<DataLayerOfferFeeFormData>({
    defaultValues: {
      fee: fee ? mojoToChik(fee).toFixed() : '',
    },
  });
  ...
  async function handleAcceptDataLayerOffer(formData: DataLayerOfferFeeFormData) {
    ...
    const feeInMojos = formData.fee ? chikToMojo(formData.fee).toFixed() : '0';
