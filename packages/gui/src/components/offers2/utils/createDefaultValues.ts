...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (XCK), indicates whether a token or CAT has a default entry
 ...
 * for creating an XCK offer, a CAT offer, or an NFT offer. XCK and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      xck: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
