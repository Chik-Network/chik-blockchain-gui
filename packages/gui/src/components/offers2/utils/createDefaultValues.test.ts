...
describe('createDefaultValues', () => {
  ...
  describe('when an XCK wallet type is provided', () => {
    it('should return an object with an XCK entry', () => {
  ...
  describe('when the returned data is modified', () => {
    it('the original data is not modified', () => {
      ...
      values.offered.xck = [{ amount: '1' }];
      values.requested.xck = [{ amount: '2' }];
      const values2 = createDefaultValues();
      expect(values2.offered.xck).toStrictEqual([]);
    });
  });
});
