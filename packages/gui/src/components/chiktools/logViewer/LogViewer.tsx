import { Flex, useShowError } from '@chik-network/core';
...
export default function LogViewer({ pageSize = 1000 }: LogViewerProps) {
  ...
  const checkLogFile = useCallback(async (): Promise<LogFileInfo> => {
    try {
      const info = await window.chikLogs.getInfo();
      if (info.fileError) {
  ...
  const loadLogsData = useCallback(
    async (forceReload = false) => {
      ...
      try {
        ...
        const result = await window.chikLogs.getContent();
        if (result.error) {
  ...
  const handleCustomPathSubmit = useCallback(
    async (path: string) => {
      try {
        await window.chikLogs.setCustomPath(path);
        const info = await checkLogFile();
        setFileInfo(info);
  ...
  const handleExport = useCallback(async () => {
    try {
      ...
      a.href = url;
      a.download = 'chik-logs-export.log';
  ...
  const content = useMemo(() => {
    ...
    if (rawContent) {
      return (
        <MonospaceBox>
          {rawContent.split('\n').map((line, index) => (
            // eslint-disable-next-line react/no-array-index-key -- Using index is safe here as the array is stable and order won't change (requested by ChikMineJP)
        ...
        <Typography color="textSecondary">
          ...
          {fileInfo.debugInfo && (
            <>
              ...
              Debug Information:
              <br />
              Chik Root: {fileInfo.debugInfo.chikRoot}
              <br />
