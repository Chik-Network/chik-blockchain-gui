...
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToChik, mojoToCAT } from '@chik-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['xck', 'txck'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToChik(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['xck', 'txck'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToChik(takerEntries[0][1])
