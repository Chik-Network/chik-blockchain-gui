...
Welcome to the Chik GUI repo!

Chik GUI is written in TypeScript and uses Electron/React.

This monorepo consists of the following packages:

| Package name  | Description                                                                                                |
| ------------- | ---------------------------------------------------------------------------------------------------------- |
| **api**       | JS/TS library to access the Chik Blockchain RPC                                                            |
...
| **icons**     | Chik specific icons                                                                                        |
| **wallets**   | Common React components and hooks. Do not use this in you project. Will be merged to **core** package soon |

## Development

1. This repo (chik-blockchain-gui) must be under chik-blockchain repo. Please follow the [installation steps for the chik-blockchain](https://github.com/Chik-Network/chik-blockchain/wiki/INSTALL#install-from-source). Make sure to install from source code (git clone...).
2. Run the `sh install-gui.sh` as instructed in the previous step. This will clone the chik-blockchain-gui under chik-blockchain repo.
3. Change directory into the chik-blockchain-gui subdirectory.
4. Run `npm run dev:gui`

**When developing, please:**

- Only edit the code with the **Vscode editor**.
- Always have **chik-blockchain-gui opened as a root folder in the Vscode** and not chik-blockchain, or chik-blockchain-gui/packages/... Failing to do so will result in incorrect auto linting and auto formatting which would not go through the CI quality checks.
- When you open the repo in the vscode, click on "**Install recommended plugins**" pop-up.
- To develop in testnet, please follow [these steps](https://github.com/Chik-Network/chik-blockchain/wiki/How-to-connect-to-the-Testnet).
...
- To install `lodash` for **single** package: `npx lerna add lodash --scope=@chik-network/icons`
...
- For Chik employees: **prefix your branch with your name**, like this: `yourName/what-is-the-code-about`. This helps when cleaning up old branches.
...
Do not edit files directly in the repo, but instead please head over to our [Crowdin project](https://chik.crowdin.com/chik-blockchain-gui) and add/edit translations there.
...
- **`npm run dev:gui` fails to start the app without providing a reason**
  ...
  1. In your command line, please go to the `chik-blockchain` directory (one level up)
  2. Run `. ./activate`
  3. Run `cd chik-blockchain-gui`
  4. Run `npm run dev:gui` to start the app.
  5. If still does not work, please open you process manager and kill all Chik / Python related processes.
...
1. In the Chik Electron app, click `View -> Developer -> Developer tools`.
...
1. Please follow the [Install and configure the simulator](https://docs.chiknetwork.com/guides/simulator-user-guide/). Do this step only once.
2. In the chik-blockchain directory, run this to setup the ENV variables. Use these instead of the ones mentioned in the above guide.

```
export CHIK_ROOT=~/.chik/simulator/main
export CHIK_SIMULATOR_ROOT=~/.chik/simulator
export CHIK_KEYS_ROOT=~/.chik_keys_simulator
```

3. `. ./activate`
4. `chik start simulator`
5. `cd chik-blockchain-gui/packages/gui`
6. `npm run dev:skipLocales`
7. You should see your simulator wallets. You should not see your testNet / mainNet wallets.
8. Run `chik dev sim farm` as many times you want to farm some coins.
